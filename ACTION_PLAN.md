...
- Freight-mode UI (air gated by `NEXT_PUBLIC_FEATURE_AIR_FREIGHT`, land gated by `NEXT_PUBLIC_FEATURE_LAND_FREIGHT`): mode filter chips + Ship/Plane/Truck row/kanban icons (`ModeIcon`) on the RFQ list; mode-aware RFQ detail (air Pieces & Dimensions + chargeable weight; land Truck Details — load type, equipment, weight-lbs, NMFC class, ZIP lane, accessorials; ocean-only blocks hidden); per-mode KPI `modeBreakdown` + "RFQs by Freight Mode" dashboard widget
- Land RFQ form (`/rfqs/new`): FTL/LTL/PTL load-type selector, truck equipment selector, weight-lbs, origin/destination ZIP, NMFC class (LTL only); `calculateFtlPrice`/`calculateLtlPrice` mirror the Python land engine; RFQ detail route loads `rfq_shipment_pieces` + `rfq_shipment_truck_details` for air/land rendering
